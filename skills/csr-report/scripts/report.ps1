...
$OutputFile = "$script:CSROutputDir\05-security-report.md"
$FinalReport = "$script:CSROutputDir\CSR-FINAL.md"

Write-CSR-Banner "PHASE 5: SECURITY REPORT & REMEDIATION PLAN"

# Check if phase files exist
Write-Host "$([char]27)[0;36m▶ Aggregating findings from Phases 1–4...$([char]27)[0m`n"
...
foreach ($Phase in $Phases) {
  $Path = Join-Path $script:CSROutputDir $Phase
  if (-not (Test-Path $Path)) {
    Write-Host "$([char]27)[1;33mWarning: $Phase not found (Phase may not have been run)$([char]27)[0m"
    $Missing++
  }
}
...
*Generated by Code Security Reviewer (CSR) Agent — $(Get-Date)*
"@
...
foreach ($Phase in $Phases) {
  $PhasePath = Join-Path $script:CSROutputDir $Phase
  if (Test-Path $PhasePath) {
    $FinalReportContent += "`n---`n"
    $FinalReportContent += (Get-Content -Path $PhasePath -Raw)
  }
}

# Append main report
$FinalReportContent += "`n---`n"
$FinalReportContent += (Get-Content -Path $OutputFile -Raw)

Set-Content -Path $FinalReport -Value $FinalReportContent -Encoding UTF8

Write-CSR-SuccessRule "Phase 5 Complete: Security Report & Remediation Plan generated."
Write-Host "$([char]27)[0;32m  Main Report: $OutputFile$([char]27)[0m"
Write-Host "$([char]27)[0;32m  Final Report: $FinalReport$([char]27)[0m"
Write-Host "`n$([char]27)[0;36mNext step: Review findings, prioritize by severity, and create remediation tickets.$([char]27)[0m"

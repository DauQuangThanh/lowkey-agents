...
**Output:** `cqr-output/01-standards-review.md` with a standards checklist, findings grouped by severity, and specific examples of violations.
...
**Output:** `cqr-output/02-complexity-report.md` with complexity metrics per function/file, hotspots identified, coupling graph, and refactoring candidates ranked by impact.
...
**Output:** `cqr-output/03-patterns-review.md` with pattern compliance checklist, SOLID audit results, DRY violation findings, separation of concerns assessment, and code smells catalog.
...
**Output:** 
- `cqr-output/04-quality-report.md` — detailed findings by category
- `cqr-output/05-cq-debts.md` — technical debt registry (CQDEBT-NN)
- `cqr-output/CQR-FINAL.md` — executive summary + quality scorecard + roadmap
...
# Methodology for Complexity Scoring
...
2. **SOLID violation cheat-sheet** — SRP: 'one reason to change' · O/C: 'adding X requires editing Y' · DI: 'new hard dependency'.
...
4. **Fallback to Interactive Mode** — If scripts are unavailable (Windows/platform issue, permissions), guide user through questions manually and write output markdown by hand
